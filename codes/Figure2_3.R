...
singular_values <- runif(R_true, 4, 4.5)
...
SNR_gaussian_3 <- norm(X_1-E_1, type = "F") / norm(E_1, type = "F")
...
SNR_heavy_3 <- norm(X_2-E_2, type = "F") / norm(E_2, type = "F")
...
singular_values <- runif(R_true, 3.7,4)
...
SNR_colored_3 <- norm(X_3-E_3, type = "F") / norm(E_3, type = "F")
...
SNR_3 <- c(SNR_gaussian_3, SNR_heavy_3, SNR_colored_3)
...
ggsave(file.path("Report_folder", "CVplots.png"), g, width = 25, height = 9.5, units = "cm")

g1 <- grid.arrange(g7cv, g8cv, g9cv, ncol=3,
                   widths = c(1.15, 1, 1))

ggsave(file.path("Report_folder", "CVplots_highnoise.png"), g1, width = 25, height = 9.5, units = "cm")
